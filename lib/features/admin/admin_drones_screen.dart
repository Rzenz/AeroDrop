...
class AdminDronesScreen extends ConsumerWidget {
                  ...
                  : SliverList(
                      delegate: SliverChildBuilderDelegate((context, index) {
                        ...
                        return DroneCard(
                              drone: drone,
                              onTap: () =>
                                  _showDroneOptions(context, drone, ref),
                            )
                            .animate(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'ID: ${drone.id} • Max Cargo: ${drone.maxPayload} kg',
                              style: TextStyle(
                                color: AppColors.textSecondaryDark,
                                fontSize: 12,
                              ),
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () {
                            ref
                                .read(droneProvider.notifier)
                                .rechargeDrone(drone.id);
                            Navigator.pop(ctx);
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Drone battery recharged to 100%!',
                                ),
                                backgroundColor: AppColors.success,
                              ),
                            );
                          ...
                          onPressed: () {
                            ...
                            if (drone.status == DroneStatus.busy) {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    'Drone "${drone.name}" sent to decommission queue.',
                                  ),
                                  backgroundColor: AppColors.success,
                                ),
                              );
  ...
  ImageFilter javaScriptBlurFilter() =>
